...
async function writeShellProfiles(env: Record<string, string>): Promise<void> {
  ...
  const bashProfileContents =
    "# Generated by Voratiq to preserve manifest env\n" +
...
function formatEnvExports(env: Record<string, string>): string {
  const lines: string[] = [];
