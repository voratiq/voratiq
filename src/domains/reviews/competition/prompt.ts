...
export function buildReviewPrompt(
  ...
  const lines: string[] = [
    ...
    ...candidateList,
    "",
    "Large diffs and context budget:",
    "- Do not use subagents for candidate analysis. Keep judgments coherent in a single pass.",
    "- Do not read entire `diff.patch` files by default. Use chunked reads (`offset/limit` or line ranges).",
    "",
    "Suggested workflow:",
    "1) Read `artifact-information.json` and `spec.md` first.",
    "2) For each candidate, inspect changed files and key symbols, then read targeted diff chunks.",
    "3) Record concise notes mapped to requirements.",
    "4) Compare candidates from notes, then write ranking and recommendation.",
    "",
